...
| `queued_at`            | user messages               | Optional. Epoch ms when the message was queued behind a running turn. Cleared when the run-state machine fires it. The set of unfired `queued_at` messages _is_ the queue. See [Turn Queue](./queue.md).                                                                       |
