...
| [Lifecycle Events](./events.md)                | The multi-subscriber session-lifecycle event channel: `turn-started` / `turn-finished` / `approval-requested` vocabulary, volatility and ordering semantics, the host-wide projected stream, the notification consumer policy (focus gating, click-to-attend), and the boundary against a user-facing hooks system.                                                                                             |
