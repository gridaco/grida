...
explicit. Command composition exposes that choice in the CLI delivery.
...
Account status and storage inspection expose only safe metadata, without
showing tokens or contacting the issuer. `grida auth logout` clears
