...
> When the selection spans **multiple parents**, there is one overlay
> per [selection partition](./selection-partition.md) — the "selection
> group overlay" this document describes is the single-partition
> (shared-parent) case. The partition, not this section, decides how
> many overlays render and which commands treat the selection as one
> host versus many.
