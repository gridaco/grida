...
Scope: **design only**. No code in this doc. The companion file
`element-ir-migration.md` sketches the implementation phasing.
...
- **Geometry mutators**: `set_local_box`, `set_view_box(box)`,
  ...
  resize-vs-rescale policy choice (`§7.5` of svg-element-model) as
...
- **Declared frame**: per-resource; see `svg-element-model.md`
...
to edit it." Per `svg-element-model.md §Hazards`.
...
`svg-transform-and-frame §3` "Round-trip caveat").
...
| `group`                     | (handled by group policy; see `grouping.md` — IR exposes `is_groupable`)    |
...
| `group` — every group-eligible element                                            | native                           | Group policy lives in `grouping.md`; IR exposes `is_groupable: bool`                                           |
...
`docs/wg/feat-svg-editor/element-ir-migration.md`. That doc is the
companion to this one; it does not duplicate the design here and
will not be drafted until this design is reviewed.
