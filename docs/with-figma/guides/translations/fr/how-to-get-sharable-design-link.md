...
1. Ouvrez le fichier Figma et allez à la frame que vous voulez partager.
2. Faites un clic droit sur la frame.
3. Ouvrez **Copy/Paste as**.
4. Sélectionnez **Copy link**.

Vous avez maintenant un lien direct vers cette frame. Vous pouvez le partager avec vos collaborateurs ou l'utiliser comme entrée dans des workflows Grida.
...
L'URL copiée peut inclure un paramètre de requête `node-id`, par exemple :

```txt
https://www.figma.com/file/XXXXXXX/example-file?node-id=0%3A1
```

Si vous voulez un lien vers tout le fichier plutôt que vers un nœud spécifique, supprimez la partie `?node-id=...` afin que l'URL ressemble à ceci :
