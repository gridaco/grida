...
| **ENGINE**            | the phase-4 layer programs (owner: browser/game-engine techniques from day 1)   | **DRAFTED 2026-07-08** — [`ENGINE.md`](./ENGINE.md): major contracts ENG-0…ENG-5 (stage purity + the oracle law · incremental resolve & damage · retained paint · spatial read tier · deterministic content oracles · time-as-data), growth paths where every stage keeps every contract, open studies OS-\*; every claim validity-tagged (`[MEASURED]`/`[PRIOR]`/`[PEER]`/`[CONTRACT]`/`[OPEN]`) — decision-independent sockets, does not pre-empt DEC-4/5/6                                |
