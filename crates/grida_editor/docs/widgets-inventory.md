...
a claim: *together with the others, this primitive is sufficient
...
4. **Composites adapt values, not events.** A composite exists only
   where a *compound value* exists. It arranges atoms and owns the
...
| section | collapsible titled group with a header-actions slot; its *presence* is a capability gate (SHEET-1) |
...
in the inventory; their *content* arrives when the capability does.
