...
fn main() {
    println!("=== Headless GPU Rendering Demo ===\n");

    let mut gpu = HeadlessGpu::new(1000, 1000).expect("GPU init");
    gpu.print_gl_info();

    let scene = create_grid_scene(70, 70);
    let mut renderer = gpu.create_renderer();
    renderer.load_scene(scene);
    println!("Loaded scene: 4900 nodes\n");

    let scenarios: Vec<(&str, f32, f32, f32)> = vec![
        ("all_visible (zoom out)", 1050.0, 1050.0, 0.45),
        ("partial (~25%)",         0.0,    0.0,    1.0),
        ("corner (~5%)",           0.0,    0.0,    2.0),
        ("zoomed_in (~1%)",        1050.0, 1050.0, 10.0),
        ("empty (offscreen)",      9999.0, 9999.0, 1.0),
    ];
    ...
    for (name, cx, cy, zoom) in &scenarios {
        ...
        println!(
            "  {name:<25} | avg {avg_frame_us:>6} \u{00b5}s ({fps:>7.1} fps) | \
             dl: {avg_dl:>5} | pic: {avg_pic:>5} | wall: {wall_ms:.1} ms",
        );
    }
