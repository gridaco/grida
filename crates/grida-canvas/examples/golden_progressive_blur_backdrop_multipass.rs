...
fn main() {
    ...
    let mut surface = surfaces::raster_n32_premul((width, height)).expect("surface");
    {
        ...
        // draw background gradient - make it smaller than the rect for better visibility
        let bg_rect = Rect::from_xywh(100.0, 100.0, 200.0, 200.0); // smaller than the 300x300 rect
        let bg_shader = Shader::linear_gradient(
            (
                Point::new(bg_rect.left(), bg_rect.top()),
                Point::new(bg_rect.left(), bg_rect.bottom()),
            ),
    ...
    let rect = Rect::from_xywh(50.0, 50.0, 300.0, 300.0);
    {
        let canvas = surface.canvas();
        canvas.save();
        ...
        // overlay semi transparent fill like backdrop-filter
        let mut fg_paint = Paint::default();
        fg_paint.set_color(Color::from_argb(0, 255, 255, 255));
