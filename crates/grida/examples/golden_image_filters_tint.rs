//! # Grida Canvas Skia - Image Tint Filter Example
//!
//! This example demonstrates the tint filter implementation using Skia color matrix.
...
//!   - gK = 1.0: neutral (no change)
//!   - gK > 1.0: more green (less magenta)
//!   - gK < 1.0: less green (more magenta)
//!
//! ## Layout
...
use grida::cg::types::ImageFilters;
use grida::painter::image_filters;
...
fn main() {
    ...
    // Draw title
    draw_centered_label(canvas, 1000.0, 30.0, "Image Filters: Tint");
    ...
    for (row, (image_file, image_label)) in images.iter().enumerate() {
        ...
        for (col, (&tint_gk, label)) in tint_values.iter().zip(tint_labels.iter()).enumerate() {
    ...
    // Save PNG
    dev_kit::save_golden(&mut surface, "image_filters_tint");
}
