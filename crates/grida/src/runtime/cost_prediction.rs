...
//! - [`crates/grida/examples/skia_bench/skia_bench_cost_model.rs`] —
//!   per-effect validation benchmark (fixed cost extraction, linearity).
//! - [`crates/grida/examples/skia_bench/skia_bench_cache_blit.rs`] —
//!   cache hit/miss ratio measurement.
//! - [`crates/grida/examples/skia_bench/skia_bench_scene_scale.rs`] —
...
pub fn estimate_node_cost(layer: &PainterPictureLayer, is_cache_hit: bool) -> f64 {
    ...
    let (effects, base) = match layer {
        ...
        PainterPictureLayer::MarkdownEmbed(m) => (&m.effects, &m.base),
