...
///
/// Per [Blink's](https://source.chromium.org/) `HarfBuzzShaper::Shape`
/// pipeline (`harfbuzz_shaper.cc`) and our research note on
/// `core/layout/svg/svg_text_layout_algorithm.cc`, this is the same
/// shaping kernel used for HTML and SVG; the SVG-specific per-character
/// positioning runs as a *post* pass on the shaped advances.
fn shape_text(text: &str, font: &Font) -> (Vec<ShapedGlyph>, Point) {
    if text.is_empty() {
