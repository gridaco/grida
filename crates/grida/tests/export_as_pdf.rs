...
use grida::export::{export_node_as, ExportAs};
use grida::node::{
    factory::NodeFactory,
...
fn test_pdf_export() {
    ...
    rect.fills = Paints::new([Paint::from(CGColor::from_rgba(255, 0, 0, 255))]);

    let rect_id = graph.append_child(Node::Rectangle(rect), Parent::Root);

    let scene = Scene {
        name: "Test Scene".into(),
        background_color: Some(CGColor::WHITE), // White background
        graph,
    };
    ...
    let geometry_cache = grida::cache::geometry::GeometryCache::from_scene(&scene, &fonts);
    ...
    // Verify it's a PDF
    match exported {
        grida::export::Exported::PDF(data) => {
