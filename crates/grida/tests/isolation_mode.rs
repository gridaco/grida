...
use grida::runtime::scene::{Backend, FrameFlushResult, Renderer};
...
fn build_two_subtree_scene() -> (Scene, NodeId, NodeId, NodeId, NodeId, NodeId) {
    ...
    cb.position = LayoutPositioningBasis::Cartesian(grida::cg::types::CGPoint { x: 200.0, y: 0.0 });
...
fn make_renderer(scene: Scene, vp_w: i32, vp_h: i32) -> Renderer {
    let mut renderer = Renderer::new_with_options(
        ...
        grida::runtime::scene::RendererOptions {
            config: grida::runtime::config::RuntimeRendererConfig {
...
fn flush_ok(renderer: &mut Renderer) -> grida::runtime::scene::FrameFlushStats {
...
fn isolate_and_flush(
    ...
    mode: IsolationMode,
) -> grida::runtime::scene::FrameFlushStats {
    renderer.set_isolation_mode(Some(mode));
    renderer.mark_global(grida::runtime::invalidation::GlobalFlag::RenderFilter);
