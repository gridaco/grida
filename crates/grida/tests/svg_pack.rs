...
use grida::formats::svg::sanitize::sanitize_svg;
use grida::import::svg::SVGPackedScene;
...
fn count_nodes(scene: &SVGPackedScene) -> usize {
    fn count(children: &[grida::cg::svg::IRSVGChildNode]) -> usize {
        children
            .iter()
            .map(|c| match c {
                grida::cg::svg::IRSVGChildNode::Group(g) => 1 + count(&g.children),
                _ => 1,
            })
            .sum()
    }
...
fn pack_nested_transforms_world_positions() {
    ...
    let viewport = grida::node::schema::Size {
        width: 500.0,
        height: 500.0,
    };
    ...
    let mut paragraph_cache = grida::cache::paragraph::ParagraphCache::new();
...
fn pack_nested_translate_positions() {
    ...
    fn collect_path_world_x(
        ...
        let node = graph.get_node(id).expect("node");
        if matches!(node, grida::node::schema::Node::Path(_)) {
            if let Some(t) = geometry.get_world_transform(id) {
                out.push(t.x());
            }
        }
...
fn svg_to_grida_fbs_gradient_transform_roundtrip() {
    ...
    let gradient = match fill {
        grida::cg::types::Paint::LinearGradient(lg) => lg,
...
fn pack_attributed_text_full_pipeline() {
    ...
    let mut paragraph_cache = grida::cache::paragraph::ParagraphCache::new();
    let text_measure = grida::layout::tree::TextMeasureProvider {
    ...
    for r in graph.roots() {
        fn dump_layout(
            graph: &grida::node::scene_graph::SceneGraph,
            layout: &grida::layout::cache::LayoutResult,
            id: &grida::node::schema::NodeId,
            depth: usize,
        ) {
            let indent = "  ".repeat(depth);
