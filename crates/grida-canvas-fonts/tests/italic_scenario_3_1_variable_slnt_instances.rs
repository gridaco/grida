...
fn test_scenario_3_1_recursive_font() {
    ...
    assert_eq!(classification.classification.italic_kind(), italic::ItalicKind::Italic);
...
fn test_scenario_3_1_true_italic_instances() {
    ...
    assert!(classification.classification.instance_info.is_some());
    let instance_info = classification.classification.instance_info.unwrap();
    assert!(instance_info.italic_instances.contains(&"Italic".to_string()));
    assert!(instance_info.italic_instances.contains(&"TrueScenario-Italic".to_string()));
...
fn test_scenario_3_1_user_font_style_override() {
    ...
    println!("✅ Scenario 3-1 (User override): Correctly classified as Italic via user declaration");
}
...
fn test_scenario_3_1_family_aggregation() {
    ...
    assert!(italic_face.vf_recipe.as_ref().unwrap().axis_values.contains_key("slnt"));
