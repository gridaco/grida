...
pub unsafe extern "C" fn text_edit_enter(
    ...
    let Some(app) = app.as_mut() else { return false };
    let Some(node_id) = __str_from_ptr_len(node_id_ptr, node_id_len) else { return false };
...
#[no_mangle]
pub unsafe extern "C" fn text_edit_is_active(
    app: *mut UnknownTargetApplication,
) -> bool {
    app.as_ref().map(|a| a.text_edit_is_active()).unwrap_or(false)
}
...
#[no_mangle]
pub unsafe extern "C" fn text_edit_get_text(
    app: *mut UnknownTargetApplication,
) -> *const u8 {
    let Some(app) = app.as_ref() else { return std::ptr::null() };
...
pub unsafe extern "C" fn text_edit_command(
    ...
    let Some(json) = __str_from_ptr_len(json_ptr, json_len) else { return };

    let cmd: WasmEditCommand = match serde_json::from_str(&json) {
        Ok(c) => c,
        Err(e) => { eprintln!("text_edit_command: {e}"); return; }
    };

    match cmd {
        WasmEditCommand::Undo => { app.text_edit_undo(); }
        WasmEditCommand::Redo => { app.text_edit_redo(); }
        other => {
...
#[no_mangle]
pub unsafe extern "C" fn text_edit_undo(
    app: *mut UnknownTargetApplication,
) -> bool {
    let Some(app) = app.as_mut() else { return false };
    app.text_edit_undo()
...
#[no_mangle]
pub unsafe extern "C" fn text_edit_redo(
    app: *mut UnknownTargetApplication,
) -> bool {
    let Some(app) = app.as_mut() else { return false };
    app.text_edit_redo()
...
pub unsafe extern "C" fn text_edit_pointer_down(
    app: *mut UnknownTargetApplication,
    x: f32, y: f32, shift: bool, click_count: u32,
) {
    let Some(app) = app.as_mut() else { return };
...
#[no_mangle]
pub unsafe extern "C" fn text_edit_pointer_up(
    app: *mut UnknownTargetApplication,
) {
    let Some(app) = app.as_mut() else { return };
...
#[no_mangle]
pub unsafe extern "C" fn text_edit_ime_cancel(
    app: *mut UnknownTargetApplication,
) {
    let Some(app) = app.as_mut() else { return };
...
pub unsafe extern "C" fn text_edit_paste_text(
    ...
    let Some(app) = app.as_mut() else { return };
    let Some(text) = __str_from_ptr_len(text_ptr, text_len) else { return };
...
pub unsafe extern "C" fn text_edit_paste_html(
    app: *mut UnknownTargetApplication,
    html_ptr: *const u8, html_len: usize,
) {
    let Some(app) = app.as_mut() else { return };
    let Some(html) = __str_from_ptr_len(html_ptr, html_len) else { return };
...
#[no_mangle]
pub unsafe extern "C" fn text_edit_get_caret_rect(
    app: *mut UnknownTargetApplication,
) -> *const u8 {
    let Some(app) = app.as_mut() else { return std::ptr::null() };
...
pub unsafe extern "C" fn text_edit_get_selection_rects(
    ...
    let Some(app) = app.as_mut() else { return std::ptr::null() };
    let Some(rects) = app.text_edit_get_selection_rects() else { return std::ptr::null() };

    #[derive(serde::Serialize)]
    struct R { x: f32, y: f32, w: f32, h: f32 }

    let json_rects: Vec<R> = rects.iter().map(|r| R {
...
pub unsafe extern "C" fn text_edit_set_font_family(
    app: *mut UnknownTargetApplication,
    family_ptr: *const u8, family_len: usize,
) {
    let Some(app) = app.as_mut() else { return };
    let Some(family) = __str_from_ptr_len(family_ptr, family_len) else { return };
...
#[no_mangle]
pub unsafe extern "C" fn text_edit_tick(
    app: *mut UnknownTargetApplication,
) -> bool {
    let Some(app) = app.as_mut() else { return false };
    app.text_edit_tick()
