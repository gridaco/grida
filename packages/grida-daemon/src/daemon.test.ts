 ...
 *   - one integration block against a REAL DaemonServer socket: the
...
import { DAEMON_PROTOCOL } from "./protocol/handshake";
...
function handshakeResponse(protocol: number = DAEMON_PROTOCOL): Response {
  return new Response(
...
describe("Daemon.probe", () => {
  it("accepts a matching-protocol handshake and sends auth + referer", async () => {
    ...
    expect(headers.authorization).toBe(
      DaemonTransport.buildBasicAuthHeader("secret")
    );
  ...
  it("gates on the wire protocol, not the version string", async () => {
    const result = await Daemon.probe(REG, "secret", {
      fetch: async () => handshakeResponse(DAEMON_PROTOCOL + 1),
    });
...
describe("Daemon probe against a real DaemonServer", () => {
  let host: DaemonServer;
  let userData: string;
  ...
  it("connects through the production perimeter end-to-end", async () => {
    ...
    await Daemon.publish(
      ...
      Daemon.mintRegistration({
        version: "0.0.0",
        url: DaemonTransport.baseUrl(host.port),
        pid: process.pid,
      })
    );
    ...
    const client = new DaemonTransport.Client({
    ...
    expect(handshake.protocol).toBe(DAEMON_PROTOCOL);
  });
