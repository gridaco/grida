 ...
 */
export function makeRefererGuard(access: DaemonHttpAccess): MiddlewareHandler {
  const allowedOrigins = new Set(access.allowed_origins);
...
function normalizePathRoot(pathRoot: string): string {
  if (!pathRoot.startsWith("/")) {
    throw new Error(`DaemonHttpAccess path must start with /: ${pathRoot}`);
  }
