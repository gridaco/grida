/**
 * Tmpdir workspace fixture for agent-tenant tests. A deliberate local
 * twin of `@grida/daemon`'s `test/daemon-fixture.ts` (test files are not
 * part of the daemon's public surface, so each package keeps its own
 * 40-line copy rather than exporting a testing subpath).
 */
import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { WorkspaceRegistry, type Workspace } from "@grida/daemon/server";
