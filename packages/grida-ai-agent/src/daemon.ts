/**
 * Daemon — the discovery contract that turns an `AgentHost` into a
...
export namespace Daemon {
  ...
  export type ProbeOptions = {
    ...
    fetch?: typeof fetch;
    timeout_ms?: number;
    /** Expected wire protocol. Defaults to `AGENT_SERVER_PROTOCOL`. */
    protocol?: number;
  };

  export type ProbeResult =
    | { ok: true; handshake: AgentServerHandshakeResponse }
    | {
        ok: false;
        reason:
          | "unreachable"
  ...
  export async function probe(
    ...
    const expected = options.protocol ?? AGENT_SERVER_PROTOCOL;
    let res: Response;
    try {
      res = await fetchImpl(`${registration.url}/handshake`, {
        method: "POST",
        headers: {
          authorization: AgentTransport.buildBasicAuthHeader(credential),
    ...
    let handshake: AgentServerHandshakeResponse;
    try {
      handshake = (await res.json()) as AgentServerHandshakeResponse;
    } catch {
  ...
  export type Connection = {
    ...
    credential: string;
    url: string;
    handshake: AgentServerHandshakeResponse;
  };
