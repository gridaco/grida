...
export namespace layout.flex {
  ...
  export function guess(
    boundingboxes: cmath.Rectangle[]
  ): Guessed & {
    union: cmath.Rectangle;
  } {
    assert(boundingboxes.length > 0, "At least one bounding box is required.");
    ...
    const totalXGap = xGaps.reduce((acc, g) => acc + g, 0);
    const totalYGap = yGaps.reduce((acc, g) => acc + g, 0);
