...
Successful results contain only generated bytes and media types. Provider
metadata, headers, URLs, warnings, and bodies are discarded. A trusted host can
separately observe fal completion as described below. `ImageClient.Failure`
...
## fal execution and completion

The image/video clients admit controls from each exact serving schema, verified
against [fal's queue OpenAPI](https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=google/gemini-omni-flash/v1.1/text-to-video).
`MediaOperations.parseInput` is a pure preflight with the same rules as native
execution; it reads no credentials and submits no jobs. GG keeps its existing
text request vocabulary so its server can choose a documented compatibility
exception before submission. An upstream failure never changes the provider.

Text-to-video maps numeric seconds to the endpoint's integer or string duration
and `WxH` dimensions to its resolution tier and aspect ratio. Unsupported
resolutions, conflicting ratios, durations, `fps`, and seeds fail before I/O.
Omni supports 3–10 seconds; Seedance 2.0 supports 4–15, and 2.5 supports 4–30.
This does not add hosted images, reference frames or audio controls to GG.

For images, GPT Image 2/2.5 accept explicit dimensions within their pixel/edge
and aspect envelopes. Flux, Seedream and Recraft map supported aspect ratios to
named `image_size` presets. Nano Banana and Grok accept native aspect ratios;
explicit SDK sizes are limited to supported square resolution tiers because
these providers do not accept arbitrary exact dimensions. Quality and seed are
withheld when the exact endpoint has no matching field. Flux and Recraft generate
one image per job; larger `n` values run sequential jobs and stop on the first
failure. Completed earlier jobs may still be billable.

Both constructors accept optional `on_fal_completed(receipt)`. This trusted,
synchronous sink receives an immutable `{ provider_id: "fal", binding_id,
request_id, output_count }` after a bounded successful result is read, before
any asset download. It runs once per completed batch, including when later
validation or delivery fails. It receives no key, prompt, URL, raw body, price or
invented usage. A thrown/async sink fails safely and cannot resubmit the job.
Hosts own billing and reconciliation; output count alone is not an exact charge.

A fal failure may carry `task_id`, a bounded accepted request identifier, in its
safe JSON representation. It can support reconciliation after timeout or failed
result delivery, but does not grant polling authority or mean generation
completed. Submission failures without an accepted identifier omit it. Public
success results remain byte-only, and ordinary clients need no completion sink.
...
GG remains text-only and follows the catalogue's explicit hosted binding. Missing
hosted metadata retains legacy Vercel eligibility; an explicit `null` withholds GG.
The SDK sends canonical model IDs to GG, which owns provider execution and
pre-submission compatibility exceptions. Explicit fal text selections use the
separately verified text-to-video binding; `image: true` retains the primary
image-to-video binding. Vercel AI Gateway Grok requires a start frame. OpenRouter uses `frame_images` with
