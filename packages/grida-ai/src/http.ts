// GRIDA-SEC-004 — scoped host transport for provider egress.
/**
...
export class ProviderHttp {
  ...
  /**
   * URL-part lowering, bound only to the download operation. Keep this
   * structural signature here: exposing the SDK's root type would pull its
   * unrelated Node transport declarations into every neutral consumer.
   */
  readonly downloadParts: (
    parts: Array<{ url: URL; isUrlSupportedByModel: boolean }>
  ) => PromiseLike<
...
function decodeBase64DataUrl(
  ...
  const buffer = Uint8Array.from(decoded, (character) =>
    character.charCodeAt(0)
  );
  assertDataUrlSize(url, buffer.byteLength, maxBytes);
  // Return a plain typed array in every supported runtime.
