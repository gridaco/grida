...
| Delete      | `Delete` or `Backspace` | `Delete` or `Backspace`     | [x]    | `vector.delete-vertex` → `selection.remove` | shipped — mode-routed (#880). In path-edit (`edit-content`) `vector.delete-vertex` removes the sub-selected vertices / segments / tangents (policy-class `delete-vertex`: `restrict` for vertex-chain, `bake` for path); the chain otherwise falls through to `selection.remove`, which deletes the selected element(s) in `select` mode |
