...
import {
  ...
  stage_pixel_grid,
  stage_snap,
} from "../src/core/resize-pipeline/stages";
import type { ResizeBaseline } from "../src/core/intents";
...
describe("resize-pipeline — rect E handle", () => {
  it("snaps right edge to a neighbor's left edge within threshold", () => {
    ...
    const out = run_resize_pipeline(plan, STAGES_DEFAULT, ctx);
...
describe("resize-pipeline — aspect lock (Shift)", () => {
  it("collapses to uniform on corner drag", () => {
    ...
    const out = stage_aspect_lock.run(plan, {
...
describe("resize-pipeline — pixel_grid stage", () => {
  it("identity when quantum is null", () => {
    ...
    const out = stage_pixel_grid.run(plan, {
      input: { id: "a", direction: "e", dx: 17.4, dy: 0 },
      modifiers: { aspect_lock: "off", force_disable_snap: false },
