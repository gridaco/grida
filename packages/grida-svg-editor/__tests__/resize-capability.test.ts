...
import {
  ...
  is_corner_direction,
  resize_constraint,
} from "../src/core/resize-capability";
import type { ResizeBaseline, ResizeDirection } from "../src/core/intents";
...
describe("direction_mask", () => {
  const cases: Array<[ResizeDirection, ReturnType<typeof direction_mask>]> = [
  ...
  for (const [dir, expected] of cases) {
    it(`${dir} → ${JSON.stringify(expected)}`, () => {
      expect(direction_mask(dir)).toEqual(expected);
    });
  }
});
...
describe("is_corner_direction", () => {
  it("returns true for corners only", () => {
    expect(is_corner_direction("nw")).toBe(true);
    expect(is_corner_direction("ne")).toBe(true);
    expect(is_corner_direction("se")).toBe(true);
    expect(is_corner_direction("sw")).toBe(true);
    expect(is_corner_direction("n")).toBe(false);
    expect(is_corner_direction("s")).toBe(false);
    expect(is_corner_direction("e")).toBe(false);
    expect(is_corner_direction("w")).toBe(false);
  });
});

describe("resize_constraint — rect (free element)", () => {
  it("is identity for rect", () => {
    const c = resize_constraint(rect_baseline(), "se", 1.5, 0.8);
...
describe("resize_constraint — text", () => {
  it("uniform on corner drags", () => {
    const c = resize_constraint(text_baseline(), "se", 1.5, 0.8);
    expect(c).toEqual({ sx: 0.8, sy: 0.8, no_op: false, uniform: true });
  });
  it("no-op on edge drags", () => {
    const c = resize_constraint(text_baseline(), "e", 1.5, 1);
    expect(c).toEqual({ sx: 1, sy: 1, no_op: true, uniform: true });
  });
  it("no-op on top/bottom edge too", () => {
    const c = resize_constraint(text_baseline(), "n", 1, 0.5);
    expect(c).toEqual({ sx: 1, sy: 1, no_op: true, uniform: true });
  });
});
...
describe("effective_resize — rect", () => {
  it("SE drag produces the gesture rect with origin at NW", () => {
    const eff = effective_resize(rect_baseline(0, 0, 100, 50), "se", 1.2, 1.4);
...
describe("effective_resize — circle (Y component dropped)", () => {
  it("SE drag with sx > sy collapses to a square sized by sy", () => {
    const eff = effective_resize(circle_baseline(50, 50, 25), "se", 1.5, 1.2);
...
describe("effective_resize — text", () => {
  it("edge drag is a no-op rect (baseline returned)", () => {
    ...
    const eff = effective_resize(b, "e", 1.5, 1);
  ...
  it("corner drag is uniform on min", () => {
    const eff = effective_resize(text_baseline(), "se", 1.5, 1.2);
    expect(eff.uniform).toBe(true);
