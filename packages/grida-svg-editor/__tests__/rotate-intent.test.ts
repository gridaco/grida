...
import { rotate_pipeline } from "../src/core/rotate-pipeline";
...
describe("is_rotatable", () => {
  it("accepts a clean rect with no transform", () => {
    ...
    expect(rotate_pipeline.intent.is_rotatable(doc, id)).toEqual({
      kind: "yes",
    });
  });
...
describe("capture_rotate_baseline + apply_rotate", () => {
  it("writes a rotate token around the pivot for a clean rect", () => {
    const { doc, id } = with_rect({ x: 0, y: 0, width: 10, height: 10 });
    const baseline = rotate_pipeline.intent.capture_baseline(doc, id, {
      x: 5,
      y: 5,
    });
    ...
    rotate_pipeline.intent.apply(doc, id, baseline, 30 * DEG);
  ...
  it("preserves the leading translate around the pivot's pre-translate space", () => {
    ...
    rotate_pipeline.intent.apply(doc, id, baseline, 90 * DEG);
  ...
  it("accumulates onto a pre-existing rotation (replaces pivot with new)", () => {
    ...
    rotate_pipeline.intent.apply(doc, id, baseline, 60 * DEG);
  ...
  it("identity-restores byte-equal when angle = 0 and no prior rotation", () => {
    ...
    rotate_pipeline.intent.apply(doc, id, baseline, 0);
  ...
  it("emits canonical 3-arg form even when pivot = origin (explicit_pivot survives re-parse)", () => {
    ...
    rotate_pipeline.intent.apply(doc, id, baseline, 45 * DEG);
...
describe("is_resizable_node — transform-aware capability check", () => {
  it("accepts a clean rect", () => {
    const { doc, id } = with_rect({ x: 0, y: 0, width: 10, height: 10 });
    expect(resize_pipeline.intent.is_resizable_node(doc, id)).toBe(true);
  });
  ...
  it("refuses user-authored `rotate(θ)` (1-arg, no explicit pivot)", () => {
    ...
    expect(resize_pipeline.intent.is_resizable_node(doc, id)).toBe(false);
  });
...
describe("resize → rotate composition (no pivot drift)", () => {
  ...
  it("rect: resize then rotate composes the angle and pivots around current center", () => {
    ...
    const resize_baseline = resize_pipeline.intent.capture_baseline(doc, id, {
      x: 60,
      y: 80,
      width: 60,
      height: 60,
    });
    resize_pipeline.intent.apply(doc, id, resize_baseline, 80 / 60, 1, {
      x: 60,
      y: 80,
    });
    ...
    const rotate_baseline = rotate_pipeline.intent.capture_baseline(
      doc,
      id,
      center
    );
    expect(rotate_baseline.current_rotation_deg).toBe(30);
    rotate_pipeline.intent.apply(doc, id, rotate_baseline, 15 * DEG);
