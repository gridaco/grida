...
describe("translate_pipeline.run", () => {
  it("is identity when stage list is empty", () => {
    const r = translate_pipeline.run(empty_plan, [], ctx());
  ...
  it("threads plan through stages in order", () => {
    ...
    const r = translate_pipeline.run(empty_plan, [a, b], ctx());
  ...
  it("skips undefined emissions", () => {
    ...
    const r = translate_pipeline.run(empty_plan, [a], ctx());
    expect(r.guides).toEqual([]);
  });
  ...
  it("does not mutate the input plan", () => {
    ...
    translate_pipeline.run(plan, [stage], ctx());
  ...
  it("is deterministic — same input gives same output", () => {
    ...
    const r1 = translate_pipeline.run(empty_plan, [stage, stage], ctx());
    const r2 = translate_pipeline.run(empty_plan, [stage, stage], ctx());
