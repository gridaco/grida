...
import {
  ...
  apply_translate,
  baseline_anchor,
  baseline_union_top_left,
  capture_resize_baseline,
  capture_translate_baseline,
...
describe("compose_leading_translate", () => {
  it("returns null for zero delta on empty input", () => {
    expect(compose_leading_translate("", 0, 0)).toBeNull();
  });

  it("emits a translate when existing is empty", () => {
    expect(compose_leading_translate("", 5, 10)).toBe("translate(5 10)");
  });

  it("merges into a leading translate without growing the string", () => {
    expect(compose_leading_translate("translate(10 20)", 5, 7)).toBe(
      "translate(15 27)"
    );
  });

  it("merges with leading translate even when other ops follow", () => {
    expect(compose_leading_translate("translate(10 20) rotate(30)", 5, 7)).toBe(
      "translate(15 27) rotate(30)"
    );
  });

  it("prepends when leading op is not translate", () => {
    expect(compose_leading_translate("rotate(30)", 5, 7)).toBe(
      "translate(5 7) rotate(30)"
    );
  });
});
...
describe("apply_translate", () => {
  it("translates a rect by writing x/y intrinsics", () => {
    ...
    const baseline = capture_translate_baseline(doc, rect_id);
    apply_translate(doc, rect_id, baseline, 5, 7);
  ...
  it("translates a circle via cx/cy", () => {
    ...
    apply_translate(doc, id, capture_translate_baseline(doc, id), -30, 40);
  ...
  it("groups translate via transform attribute and never bloat across drags", () => {
    ...
    const id = doc.element_children_of(doc.root)[0];
    let baseline = capture_translate_baseline(doc, id);
    apply_translate(doc, id, baseline, 10, 0);
  ...
  it("rotated group stays bounded at one extra term", () => {
    ...
    const id = doc.element_children_of(doc.root)[0];
    apply_translate(doc, id, capture_translate_baseline(doc, id), 10, 0);
...
describe("compute_resize_factors", () => {
  ...
  it("se drag scales positive from nw anchor", () => {
    const f = compute_resize_factors(baseline, "se", 50, 25, false);
  ...
  it("east edge handle only affects x", () => {
    const f = compute_resize_factors(baseline, "e", 30, 999, false);
    expect(f.sx).toBeCloseTo(1.3, 5);
    expect(f.sy).toBe(1);
  });

  it("shift on corner locks aspect to max", () => {
    const f = compute_resize_factors(baseline, "se", 20, 5, true);
...
describe("apply_resize", () => {
  it("scales a rect's x/y/width/height around the given origin", () => {
    ...
    const baseline = capture_resize_baseline(doc, rect_id, {
      x: 10,
      y: 10,
      width: 100,
      height: 50,
    });
    apply_resize(doc, rect_id, baseline, 2, 2, { x: 10, y: 10 });
  ...
  it("clamps circle to uniform scale via min(sx, sy)", () => {
    ...
    const id = doc.element_children_of(doc.root)[0];
    const baseline = capture_resize_baseline(doc, id, {
      x: 30,
      y: 30,
      width: 40,
      height: 40,
    });
    apply_resize(doc, id, baseline, 3, 2, { x: 0, y: 0 });
...
describe("apply_resize — rotated rect: visual position tracks gesture, not drifts", () => {
  it("SE-anchored resize keeps NW corner pinned in doc space", () => {
    ...
    apply_resize(doc, rect_id, baseline, 80 / 60, 1, { x: 60, y: 80 });
  ...
  it("multi-frame SE drag: NW pinned across cumulative frames (no drift accumulation)", () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      apply_resize(doc, rect_id, baseline, target_w / cur_w, 1, {
        x: parseFloat(doc.get_attr(rect_id, "x") ?? "0"),
        y: parseFloat(doc.get_attr(rect_id, "y") ?? "0"),
      });
    }
...
describe("apply_resize — rotate pivot renormalization (commit-time)", () => {
  ...
  it("preserves leading translate verbatim across compensation", () => {
    ...
    apply_resize(doc, rect_id, baseline, 2, 1, { x: 0, y: 0 });
  ...
  it("leaves transform=null untouched on un-rotated rect", () => {
    ...
    apply_resize(doc, rect_id, baseline, 1.5, 1.5, { x: 0, y: 0 });
  ...
  it("zero-delta resize on rotated rect: transform byte-identical", () => {
    ...
    apply_resize(doc, rect_id, baseline, 1, 1, { x: 60, y: 80 });
  ...
  it("apply_resize leaves user-authored rotate(θ) verbatim (gate refuses)", () => {
    ...
    apply_resize(doc, rect_id, baseline, 1.5, 1, { x: 0, y: 0 });
  ...
  it("circle: pivot lands at the new (cx, cy)", () => {
    ...
    apply_resize(doc, id, baseline, 2, 2, { x: 0, y: 0 });
  ...
  it("ellipse: pivot lands at the new (cx, cy)", () => {
    ...
    apply_resize(doc, id, baseline, 1, 2, { x: 0, y: 0 });
    const cx = parseFloat(doc.get_attr(id, "cx") ?? "0");
    const cy = parseFloat(doc.get_attr(id, "cy") ?? "0");
    const rot = rotate_args(doc, id)!;
    expect(rot.cx).toBeCloseTo(cx, 4);
  ...
  it("line: pivot lands at the new midpoint of endpoints", () => {
    ...
    apply_resize(doc, id, baseline, 2, 1, { x: 0, y: 0 });
...
describe("baseline_anchor", () => {
  it("returns (x, y) for rect / text / tspan / image / use", () => {
    expect(baseline_anchor({ type: "rect", x: 5, y: 10 })).toEqual({
      x: 5,
      y: 10,
    });
    expect(baseline_anchor({ type: "text", x: 3, y: 4 })).toEqual({
      x: 3,
      y: 4,
    });
  });
  ...
  it("returns (cx, cy) for circle / ellipse", () => {
    expect(baseline_anchor({ type: "circle", cx: 10, cy: 20 })).toEqual({
      x: 10,
      y: 20,
    });
    expect(baseline_anchor({ type: "ellipse", cx: 1, cy: 2 })).toEqual({
      x: 1,
      y: 2,
    });
  });
  ...
  it("returns min of endpoints for line", () => {
    expect(
      baseline_anchor({ type: "line", x1: 30, y1: 5, x2: 10, y2: 50 })
    ).toEqual({ x: 10, y: 5 });
  });

  it("parses polyline / polygon points", () => {
    expect(
      baseline_anchor({ type: "polyline", points: "30,5 10,50 20,15" })
    ).toEqual({ x: 10, y: 5 });
  });

  it("parses first M command for path", () => {
    expect(baseline_anchor({ type: "path", d: "M 12.5 7 L 100 200" })).toEqual({
      x: 12.5,
      y: 7,
    });
  });
  ...
  it("uses leading-translate as anchor for viaTransform", () => {
    ...
    expect(
      baseline_anchor({ type: "viaTransform", transform: "translate(10 20)" })
    ).toEqual({ x: 10, y: 20 });
    expect(
      baseline_anchor({
        type: "viaTransform",
    ...
    expect(
      baseline_anchor({ type: "viaTransform", transform: null })
    ).toBeNull();
    // Non-translate-leading or unparseable transforms → null.
    expect(
      baseline_anchor({ type: "viaTransform", transform: "rotate(15)" })
    ).toBeNull();
    expect(
      baseline_anchor({
        type: "viaTransform",
    ...
    expect(baseline_anchor({ type: "unsupported" })).toBeNull();
  });
});
...
describe("baseline_union_top_left", () => {
  it("takes the min over all anchors", () => {
    ...
    expect(baseline_union_top_left(m)).toEqual({ x: 2, y: 10 });
  });
  ...
  it("ignores baselines with no anchor (anchorless viaTransform / unsupported)", () => {
    ...
    expect(baseline_union_top_left(m)).toEqual({ x: 5, y: 10 });
  });
  ...
  it("returns null when no baseline yields an anchor", () => {
    ...
    expect(baseline_union_top_left(m)).toBeNull();
  });
  ...
  it("includes viaTransform leading-translate anchors in the union", () => {
    ...
    expect(baseline_union_top_left(m)).toEqual({ x: 5, y: 20 });
  });
});
