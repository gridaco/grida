...
import {
  ...
  STAGES_DEFAULT as TRANSLATE_STAGES_DEFAULT,
  ...
  (zoom) => {
    it("integer width across a fractional cursor sweep", () => {
      ...
      for (let cursor_px = 10; cursor_px <= 30; cursor_px += 0.137) {
        ...
        const { plan } = run_resize_pipeline(plan0, STAGES_DEFAULT, ctx);
        applyResizePlan(doc, plan);
...
describe.each(ZOOMS)(
  ...
  (zoom) => {
    it("integer x across a fractional cursor sweep", () => {
      const { editor, id } = build_editor_with_rect();
      const doc = doc_of(editor);
      const baselines = capture_translate_baselines(doc, [id]);
      ...
      for (let cursor_px = 10; cursor_px <= 30; cursor_px += 0.137) {
        ...
        const { plan } = run_translate_pipeline(
          plan0,
          TRANSLATE_STAGES_DEFAULT,
          ctx
        );
