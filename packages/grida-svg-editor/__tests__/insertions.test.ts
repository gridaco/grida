...
describe("insertions / drag math", () => {
  describe("rect", () => {
    it("plain drag — top-left at min, width/height at abs delta", () => {
      ...
      const b = { x: 70, y: 90 };
      expect(
        compute_drag_attrs("rect", a, b, { shift: false, alt: false })
      ).toEqual({
        x: "50",
        y: "50",
        width: "20",
    ...
    it("Shift: width === height (square; larger axis wins)", () => {
      const a = { x: 0, y: 0 };
      const b = { x: 30, y: 70 };
      const attrs = compute_drag_attrs("rect", a, b, {
        shift: true,
        alt: false,
      });
    ...
    it("Alt: anchor is center (rect grows symmetrically)", () => {
      ...
      const b = { x: 130, y: 140 };
      expect(
        compute_drag_attrs("rect", a, b, { shift: false, alt: true })
      ).toEqual({
        x: "70",
        y: "60",
        width: "60",
  ...
  describe("ellipse", () => {
    it("plain drag — bbox corners; cx/cy at midpoint, rx/ry at half-extent", () => {
      ...
      const b = { x: 40, y: 20 };
      expect(
        compute_drag_attrs("ellipse", a, b, { shift: false, alt: false })
      ).toEqual({
        cx: "20",
        cy: "10",
        rx: "20",
    ...
    it("Shift: rx === ry (uniform circle)", () => {
      const a = { x: 0, y: 0 };
      const b = { x: 60, y: 20 };
      const attrs = compute_drag_attrs("ellipse", a, b, {
        shift: true,
        alt: false,
      });
    ...
    it("Alt: anchor is center, pointer is radius", () => {
      ...
      const b = { x: 150, y: 130 };
      expect(
        compute_drag_attrs("ellipse", a, b, { shift: false, alt: true })
      ).toEqual({
        cx: "100",
        cy: "100",
        rx: "50",
  ...
  describe("line", () => {
    it("plain drag — anchor → current", () => {
      ...
      const b = { x: 50, y: 80 };
      expect(
        compute_drag_attrs("line", a, b, { shift: false, alt: false })
      ).toEqual({
        x1: "10",
        y1: "20",
        x2: "50",
    ...
    it("Shift: angle quantized to 0/45/90", () => {
      ...
      const b = { x: 5, y: 50 };
      const attrs = compute_drag_attrs("line", a, b, {
        shift: true,
        alt: false,
      });
    ...
    it("Alt: anchor is midpoint (line mirrors)", () => {
      ...
      const b = { x: 130, y: 110 };
      expect(
        compute_drag_attrs("line", a, b, { shift: false, alt: true })
      ).toEqual({
        x1: "70",
        y1: "90",
        x2: "130",
  ...
  describe("initial_attrs (zero-size at click point)", () => {
    it("rect: zero w/h at click point", () => {
      expect(initial_attrs("rect", { x: 10, y: 20 })).toEqual({
        x: "10",
        y: "20",
        width: "0",
        height: "0",
    ...
    it("ellipse: zero radii at click point", () => {
      expect(initial_attrs("ellipse", { x: 10, y: 20 })).toEqual({
        cx: "10",
        cy: "20",
        rx: "0",
        ry: "0",
    ...
    it("line: zero length at click point", () => {
      expect(initial_attrs("line", { x: 10, y: 20 })).toEqual({
        x1: "10",
        y1: "20",
        x2: "10",
        y2: "20",
  ...
  describe("default_attrs (click-no-drag commit, default-sized)", () => {
    it("rect: 100×100 centered on click", () => {
      expect(default_attrs("rect", { x: 100, y: 100 })).toEqual({
        x: "50",
        y: "50",
        width: "100",
        height: "100",
    ...
    it("ellipse: 100×100 circle centered on click", () => {
      expect(default_attrs("ellipse", { x: 100, y: 100 })).toEqual({
        cx: "100",
        cy: "100",
        rx: "50",
        ry: "50",
    ...
    it("line: horizontal 100-px segment centered on click", () => {
      expect(default_attrs("line", { x: 100, y: 100 })).toEqual({
        x1: "50",
        y1: "100",
        x2: "150",
        y2: "100",
...
describe("gesture ordering (matches main canvas pattern)", () => {
  ...
  it("commands.insert (click-no-drag analog) yields one rect, one undo step", () => {
    ...
    editor.commands.insert(
      "rect",
      // default_attrs(rect, {100, 100}) shape — 100x100 centered.
