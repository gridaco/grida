...
import { capture_resize_baseline } from "../src/core/intents";
...
describe("applyResizePlan / revertResizePlan — multi-member", () => {
  it("scales two rects around the union NW; per-member arms execute", () => {
    ...
    const ba = capture_resize_baseline(doc, a, {
      x: 0,
      y: 0,
      width: 10,
      height: 10,
    });
    const bb = capture_resize_baseline(doc, b, {
      x: 20,
      y: 0,
      width: 10,
      height: 10,
    ...
    const plan: ResizePlan = {
      id: a,
      baseline: synthesize_group_baseline(union),
      members: [
        { id: a, baseline: ba },
        { id: b, baseline: bb },
      ],
  ...
  it("circle stays uniform under multi-resize (per-tag arm fires)", () => {
    ...
    const br = capture_resize_baseline(doc, r, {
      x: 0,
      y: 0,
      width: 10,
      height: 20,
    });
    const bc = capture_resize_baseline(doc, c, {
      x: 25,
      y: 5,
      width: 10,
      height: 10,
