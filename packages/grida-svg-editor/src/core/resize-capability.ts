...
export type ResizeConstraint = {
   ...
   *  for circle / text-on-corner this is `(s, s)` with `s = min(sx, sy)`. */
  sx: number;
  sy: number;
  /** True when the gesture has no visible effect (text-on-edge). */
  no_op: boolean;
  /** True when the constraint forces `sx === sy` (circle, text-on-corner). */
  uniform: boolean;
};
...
export type DirectionMask = {
  affects_x: boolean;
  affects_y: boolean;
  /** The moving x edge of the bbox during this drag. `null` for N/S edges
   *  (x is fixed). */
  x_edge: "left" | "right" | null;
  /** The moving y edge of the bbox during this drag. `null` for E/W edges. */
...
export function is_corner_direction(dir: ResizeDirection): boolean {
 ...
 *   - `unsupported`: no-op.
 */
export function resize_constraint(
 ...
 *  axis-aligned rect plus the constrained scale factors and a moving-edge
 *  description for the snap stage. */
export type EffectiveResize = {
  rect: Rect;
  /** Post-constraint scale factors (== gesture for free elements; uniform
   *  for circle / text-on-corner; `(1, 1)` for text-on-edge no-op). */
  sx: number;
  sy: number;
 ...
 */
export function effective_resize(
  baseline: ResizeBaseline,
  dir: ResizeDirection,
  sx_gesture: number,
  sy_gesture: number
