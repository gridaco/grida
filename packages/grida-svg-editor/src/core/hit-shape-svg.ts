...
//
// Derives `HitShape` from `SvgDocument` attributes. Thin and
// intentionally fragile — this is the "backend" side of the hit-test
// split. The reusable logic (types, distance math, picker, memoizer)
// lives in `./hit-shape.ts`. The DOM-coupled wiring that composes this
// adapter with live `bounds_of` lives in `dom.ts` (`SvgHitShapeDriver`).
//
// ─── Known issues ───────────────────────────────────────────────────────
//
// See `docs/wg/feat-svg-editor/hit-test.md` for the full postmortem and
// v2 design guide. Summary:
//
...
//      against a world-space click will get junk distance for
//      transformed descendants. Mitigations: compose CTM in user code,
//      or fall back to bounds-rect when any ancestor has a transform
//      (the current dom.ts wiring does the latter, implicitly, via
//      `SvgHitShapeDriver` returning `null` here → bounds-fallback
//      there).
//
//   2. `<path>` is approximated as a control polyline: M/L endpoints,
//      plus Q/C control points (no curve sampling). Tight on straight
//      runs, loose on tightly-curved Beziers. v2 should lower curves to
//      flattened polylines or implement true distance-to-curve.
//
...
//
// The right v2 architecture is in
// docs/wg/feat-svg-editor/hit-test.md §"Concrete v2 guide".
...
export namespace hit_shape_svg {
   ...
   * `transform=` we can't compose cleanly — matrix / scale / skew /
   * mixed).
   *
   * For `transform=` values that classify to identity / leading-
   * translate / single-rotate / translate-then-rotate, the returned
   * shape is the post-transform polygon/segment (rotated rects become
   * 4-corner polygons), so picking lands on the rendered outline rather
   * than the silent AABB fallback in `SvgHitShapeDriver`.
   *
   * Caveat: see "Known issues" #1 at the top of this file — ancestor
   * CTM is still NOT composed here. Inside a `<g transform="...">` the
   * shape is still in the ancestor-relative frame.
   */
  export function of_doc(doc: SvgDocument, id: NodeId): HitShape | null {
    ...
    switch (tag) {
      ...
      case "ellipse": {
        ...
        // Rotated ellipse has no axis-aligned ellipse representation.
        // Fall back to AABB for the rotated case; translate-only stays
        // clean.
  ...
  /** Affine reduction of a classified transform list: extract the
   *  (single) rotation and (single, leading) translation. The
   *  classifier rules out multi-rotate / matrix / scale / skew before
   *  we reach here. */
  type Affine = {
    translate: Vec2;
    pivot: Vec2;
   ...
   *  any control points it carries (Q: x1/y1; C: x1/y1 + x2/y2) plus
   *  its endpoint. T/S shorthand and A arcs contribute only the
   *  endpoint at this fidelity. Returns `[]` for unparseable input. */
