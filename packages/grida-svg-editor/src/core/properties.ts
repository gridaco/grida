...
//
//   computed  — the type-parsed value, ready for the inspector. For
//               unknown property names, `computed` mirrors `declared` as a
//               string. For known names, we type-parse (numbers for
//               opacity, paint objects for fill/stroke, etc.).
//
// Out of scope for v0 (per README): `<style>` block resolution.
// Declarations from `<style>` blocks fall through to `defaulted` /
...
export namespace properties {
  ...
  /**
   * Resolve a property's declared value and its provenance for a single
   * node.
   *
   * The cascade engine here covers what the README says is in scope:
   * presentation attributes + inline style + parent inheritance +
   * initial. `<style>` block matching is deferred.
   */
  export function resolve_declared(
    ...
    // 2. presentation attribute (same name as the CSS property for SVG
    //    attrs that double as CSS properties).
  ...
  /**
   * Type-parsed computed value for known properties. Unknown property
   * names return the declared string as-is.
   */
   ...
   *  whichever carrier currently wins the cascade. If nothing wins
   *  (defaulted / inherited), write a presentation attribute by
   *  default. */
  ...
  /**
   * Structural equality on the typed-cache PropertyValue. Drives
   * reference-stable reads from `editor.node_properties`: if a rebuilt
   * value tests equal to the cached one, the prior reference is reused.
   * `computed` is either a primitive (known properties: number for
   * opacity, etc.), the raw string (unknown properties), or an
   * `InvalidComputedValue` record.
   */
  export function value_equals(a: PropertyValue, b: PropertyValue): boolean {
    if (a === b) return true;
