...
import { plan_group } from "./group";
import { prepare_translate_rpc, STAGES_NUDGE } from "./translate-pipeline";
import type { TranslateStage } from "./translate-pipeline";
import { prepare_rotate_rpc } from "./rotate-pipeline";
import {
...
import {
  apply_resize,
  apply_rotate as apply_rotate_intent,
  apply_translate,
...
import { parse_paint, serialize_paint } from "./paint";
import {
  choose_write_carrier,
  read_property,
  resolve_declared,
} from "./properties";
import type {
...
import { default_paint_attrs } from "./insertions";
...
export type SvgEditor = ReturnType<typeof createSvgEditor>;
...
export function createSvgEditor(opts: CreateSvgEditorOptions) {
  ...
  type ReadProperty = ReturnType<typeof read_property>;
  ...
  function node_paint(id: NodeId, channel: "fill" | "stroke"): PaintValue {
    ...
    const computed = parse_paint(declared);
  ...
  function write_property(id: NodeId, name: string, value: string | null) {
    const carrier = choose_write_carrier(doc, id, name);
  ...
  function set_paint(channel: "fill" | "stroke", paint: Paint) {
    if (selection.length === 0) return;
    const value = serialize_paint(paint);
  ...
  function preview_paint(channel: "fill" | "stroke"): PaintPreviewSession {
    ...
    return {
      update: (paint: Paint) => session.update(serialize_paint(paint)),
  ...
  function nudge(delta: { dx: number; dy: number }) {
    if (do_translate_oneshot(delta, STAGES_NUDGE, "nudge")) {
      notify_translate_commit();
    }
  }
  ...
  function resize_to(
    ...
    const apply = () => {
      for (const m of members) {
        apply_resize(doc, m.id, m.rz, sx, sy, origin);
      }
      if (dx !== 0 || dy !== 0) {
  ...
  function rotate_to(
    ...
    const apply = () => {
      for (const m of probe.plan.members) {
        ...
        apply_rotate_intent(doc, m.id, m.baseline, delta);
      }
      emit();
    };
    const revert = () => {
      for (const m of probe.plan.members) {
        apply_rotate_intent(doc, m.id, m.baseline, 0);
      }
      emit();
    };
    apply();
  ...
  function flatten_transform(opts?: { ids?: ReadonlyArray<NodeId> }): boolean {
    ...
    if (members.length === 0) return false;
    // Compose all ops into a single matrix per member. Identity matrix
    // is [1, 0, 0, 1, 0, 0] (row-major a, b, c, d, e, f for SVG matrix).
    type Mat = [number, number, number, number, number, number];
    const IDENT: Mat = [1, 0, 0, 1, 0, 0];
    function mul(m1: Mat, m2: Mat): Mat {
      // SVG matrix multiplication: m1 * m2 = [
    ...
    function op_to_mat(op: TransformOp): Mat {
      switch (op.type) {
        case "matrix":
  ...
  function align(
    ...
    const revert = () => {
      ...
      for (const m of members) {
        if (deltas.has(m.id)) apply_translate(doc, m.id, m.baseline, 0, 0);
      }
      emit();
    };
    apply();
  ...
  function default_paint_attrs_for(tag: string): Record<string, string> {
    if (tag === "rect" || tag === "ellipse" || tag === "line") {
      return default_paint_attrs(tag);
    }
    return {};
  }
  ...
  const public_editor = {
    ...
    // Surface bridge — drivers the DOM surface registers on attach, plus
    // the history/emit hooks the surface needs to drive preview sessions
    // (drag, resize, set-endpoint). Not part of the public API.
    _internal: {
      doc,
      history: {
      ...
      /** Drag-commit publisher; nudge/translate commands publish directly. */
      notify_translate_commit,
      ...
      /** Surface registers its geometry provider on attach; clears on detach. */
...
// Used inside the typed-read caches (`node_paint`, `node_properties`) to
// decide whether a rebuilt snapshot can reuse the prior reference. Kept
// at module scope (not exported) so the cache logic above stays terse;
// not part of the public API — consumers should rely on referential
// stability from the cache, not on these helpers.

function paint_value_equals(a: PaintValue, b: PaintValue): boolean {
  if (a === b) return true;
...
function property_value_equals(
  a: ReturnType<typeof read_property>,
  b: ReturnType<typeof read_property>
): boolean {
  if (a === b) return true;
  if (a.declared !== b.declared) return false;
  if (a.provenance.carrier !== b.provenance.carrier) return false;
  if (a.provenance.origin !== b.provenance.origin) return false;
  // computed is a primitive or an InvalidComputedValue
  if (a.computed === b.computed) return true;
  if (
    a.computed &&
    b.computed &&
    typeof a.computed === "object" &&
    typeof b.computed === "object" &&
    "error" in a.computed &&
    "error" in b.computed
  ) {
    return (
      a.computed.error === b.computed.error &&
      a.computed.reason === b.computed.reason
    );
  }
  return false;
}
