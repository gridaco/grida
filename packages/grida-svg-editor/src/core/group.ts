...
//
// `plan_group` is read-only — it validates against the policy in
// `docs/wg/feat-svg-editor/grouping.md` and either returns a `GroupPlan` (positions captured for
// apply + revert) or `null` (rejected). The editor orchestrator in
// `core/editor.ts` is the only place that creates the new `<g>` and
// drives history.
 ...
 */
export const STRUCTURAL_GRAPHICS_SET: ReadonlySet<string> = new Set([
  "g",
  "defs",
  "svg",
  "use",
 ...
 */
export const CONSTRAINED_PARENT_SET: ReadonlySet<string> = new Set([
  "text",
  "tspan",
  "defs",
  "clipPath",
...
export function plan_group(
  ...
  // Single pass: validate same-parent + valid tag.
  for (const id of ids) {
