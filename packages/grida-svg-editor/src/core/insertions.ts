...
/** Modifiers honored by the insertion drag. Shift constrains aspect / angle;
 *  Alt treats the anchor as center instead of corner. They compose. */
export type DragModifiers = { shift: boolean; alt: boolean };

/** Default size used for click-no-drag commit and as the implicit "size" for
 *  defaults_attrs callers that don't supply one. CSS px in world space. */
...
export function default_paint_attrs(
  ...
  switch (tag) {
    ...
    case "line":
      // Without a stroke a <line> renders nothing. Pick something
      // reasonable: 1px black stroke.
...
function rect_attrs(
  ...
  // Shift constrain: width === height. Pick the larger magnitude as the
  // shared size; preserve the sign of each delta so the rect can still
  // flip across the anchor.
  if (mods.shift) {
    const m = Math.max(Math.abs(dx), Math.abs(dy));
    // Sign defaults to +m on an exactly-on-axis drag (Math.sign(0) === 0).
    dx = dx < 0 ? -m : m;
    dy = dy < 0 ? -m : m;
  }
  ...
  if (mods.alt) {
    // Anchor is center; rect grows symmetrically.
    x = anchor.x - Math.abs(dx);
    y = anchor.y - Math.abs(dy);
    w = Math.abs(dx) * 2;
    h = Math.abs(dy) * 2;
  } else {
    // Anchor is a corner; the opposite corner follows the pointer.
...
function ellipse_attrs(
  ...
  if (mods.alt) {
    // Anchor is center; the pointer offset becomes the radius directly.
    cx = anchor.x;
    cy = anchor.y;
    rx = Math.abs(dx);
    ry = Math.abs(dy);
  } else {
    // Anchor is one corner of the bbox; the opposite corner is current.
    // Center is the midpoint; radii are half the bbox extents.
    cx = anchor.x + dx / 2;
    cy = anchor.y + dy / 2;
    rx = Math.abs(dx) / 2;
    ry = Math.abs(dy) / 2;
...
function line_attrs(
  ...
  // Shift: quantize to 0° / 45° / 90°. Round atan2 to nearest 45° and
  // project the drag distance along that axis.
  if (mods.shift) {
  ...
  if (mods.alt) {
    // Anchor is midpoint; line extends both directions.
    x1 = anchor.x - dx;
    y1 = anchor.y - dy;
    x2 = anchor.x + dx;
    y2 = anchor.y + dy;
  } else {
    // Anchor is the start; current is the end.
    x1 = anchor.x;
    y1 = anchor.y;
    x2 = anchor.x + dx;
    y2 = anchor.y + dy;
