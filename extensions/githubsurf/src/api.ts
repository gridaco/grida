...
export const readGitlabDirectory = async (uri : vscode.Uri) => {
	...
	if (ownerId === null) {
		return fetch(`https://gitlab.com/api/v4/projects/${await getGitlabGroupProjectId(state)}/repository/tree?per_page=100&path=${state.path !== "/" ? state.path.substr(1) : "/"}`)
			.then(r => r.map(data => {
				data["sha"] = data.id;
				delete data.id;
				data.path = data.path.split("/")[data.path.split("/").length - 1];
				return data;
			}))
			.catch(handleRequestError);
	} else {
		return fetch(`https://gitlab.com/api/v4/projects/${await getGitlabPersonalProjectId(state)}/repository/tree?per_page=100&path=${state.path !== "/" ? state.path.substr(1) : "/"}`)
			.then(r => r.map(async data => {
				data["sha"] = data.id;
				delete data.id;
				data.path = data.path.split("/")[data.path.split("/").length - 1];
				return data;
