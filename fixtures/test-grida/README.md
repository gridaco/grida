...
- **`L0.grida`**: Multi-scene raw FlatBuffer fixture generated by the Rust test suite (`cargo test --package cg --test fbs_fixtures -- write_l0 --ignored`). Exercises every node type, paint variant, effect, stroke option, mask type, and layout configuration across multiple scenes. In-memory round-trip coverage is in `cargo test --package cg --test fbs_roundtrip`.
