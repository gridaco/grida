...
- **`.grida` files**: Modern format. Two sub-variants:
  ...
  - **Raw FlatBuffer**: Bare FlatBuffers binary (no ZIP wrapper). Used for Rust decoder unit tests (`crates/grida/src/io/io_grida_fbs.rs`).
...
- **`L0.grida`**: Multi-scene raw FlatBuffer fixture generated by the fixture tool (`cargo run --package grida --example tool_gen_fixtures`). Exercises every node type, paint variant, effect, stroke option, mask type, and layout configuration across multiple scenes. In-memory round-trip coverage is in `cargo test --package grida --test fbs_roundtrip`.
- **`bench.grida`**: Heavy benchmark fixture (ZIP/FlatBuffer) generated by `cargo run --package grida --example tool_gen_bench_fixture`. Contains 15 large loop-generated scenes (~64 600 nodes, ~4.9 MB zipped) designed for benchmarking rendering, layout, cache, and compositor performance:
...
- **0.91.0-beta+20260311** ⚠️ Breaking FBS schema change:
  ...
  - Added Rust FlatBuffers decoder: `crates/grida/src/io/io_grida_fbs.rs`.
