...
2. Select two references from the initial results.
   - Expected: both thumbnails appear in the selected-reference strip.
3. Enter a materially different query and press Enter.
...
4. Select one reference from the new results.
   - Expected: the picker now shows three selected thumbnails.
...
7. Repeat the flow, starting a slow search and immediately submitting another
   query.
   - Expected: only the newest query's results appear; no rows from the older
...
9. Start one more reference request and close the picker tab.
   - Expected: closing the tab resolves the pending call without references.
10. Start another reference request and choose the compact Skip action beside
    the pending-reference chip above the composer.
    - Expected: Skip resolves the pending call without references, just like
      closing the picker tab.
