...
`$X.XX` balance. Current badges are Free, Pro, and Custom. A historical
Team subscription remains readable as **Team (legacy)**. The card is
**read-only**: the single control, **Manage
...
2. Expected: Credits card renders a skeleton, then the org's display
   name, its Free or Pro plan badge, and the granted balance as `$5.00`.
3. Repeat with an organization whose `is_enterprise` flag is true.
4. Expected: the badge is **Custom**, regardless of the underlying Stripe
   subscription tier. No fixed seat allowance is implied.
5. Repeat with a historical Team subscription and `is_enterprise=false`.
6. Expected: the badge is **Team (legacy)** rather than a saleable Team offer.
7. Click **Manage billing**.
8. Expected: the OS browser opens `/organizations/{slug}/settings/billing`
   for that org; the desktop window stays on Settings (no webview
   navigation).
9. With an org that was never provisioned for AI credits, open Settings.
10. Expected: balance renders **"—"** (not "$0.00") with the "not set up
    yet" hint; Manage billing still works.
11. Sign out (Account card) and revisit Settings.
12. Expected: Credits card shows "Sign in to see your AI credits." — no
    error state.
13. Remove `METRONOME_API_TOKEN` from `editor/.env.local` (restart dev
    server) and open Settings while signed in.
14. Expected: the card still renders the cached balance (no error); the
