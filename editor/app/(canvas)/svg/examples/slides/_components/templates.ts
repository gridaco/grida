...
export type SlideTemplate = {
  ...
  /** CSS `aspect-ratio` value parsed from the SVG's viewBox, e.g. `"1920 / 1080"`. */
  aspectRatio: string;
};
...
/**
 * Parse a CSS `aspect-ratio` value from the SVG source. Reads `viewBox`
 * first, falls back to `width`/`height` attrs; defaults to 16:9.
 */
export function aspectRatioFromSvg(svg: string): string {
  const vb = svg.match(
    /viewBox\s*=\s*"\s*[\d.+-]+\s+[\d.+-]+\s+([\d.+-]+)\s+([\d.+-]+)/
  );
  if (vb) return `${vb[1]} / ${vb[2]}`;
  const w = svg.match(/\swidth\s*=\s*"(\d+(?:\.\d+)?)"/);
  const h = svg.match(/\sheight\s*=\s*"(\d+(?:\.\d+)?)"/);
