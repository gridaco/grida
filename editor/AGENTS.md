...
| `www/`        | Public-site components for `(www)`          | [`app/(www)/README.md`](<app/(www)/README.md>) | Header/footer + landing components. Nav config: [`www/data/sitemap.ts`](www/data/sitemap.ts) (despite the name, it’s **not** `sitemap.xml`). The `(www)` route register owns those pages' intent, CTA/gate funnels, crawl state, and lifecycle. |
| `components/` | Shared UI building blocks                   | [`components/AGENTS.md`](components/AGENTS.md) | Route-agnostic, override-friendly components. Base shadcn primitives now live in the `@app/ui` package (`packages/ui`); this holds editor-local sets (`ui-editor/`, `ui-forms/`, `ui2/`) and other shared components.                           |
| `scaffolds/`  | Feature-sized UI assemblies                 | —                                              | Bigger, feature-scoped assemblies (often app-coupled).                                                                                                                                                                                          |
| `lib/`        | Stable, non-opinionated modules             | —                                              | Good candidates to promote to `/packages` once matured. Web AI gate topology: [`lib/ai/README.md`](lib/ai/README.md).                                                                                                                           |
...
| `(www)`       | Public marketing / SEO pages                           | [`app/(www)/README.md`](<app/(www)/README.md>)       | Public `grida.co` pages. Read the route register and the [`SEO skill`](../.agents/skills/seo/SKILL.md) before changing route intent, CTA/gates, funnels, crawl state, or lifecycle. |
...
- **Canonical `(www)` route register**: [`app/(www)/README.md`](<app/(www)/README.md>) (human intent, CTA/gate funnel, crawl state, and lifecycle for that route group)
