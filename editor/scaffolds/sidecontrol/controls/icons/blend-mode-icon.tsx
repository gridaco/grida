...
/**
 * Host wrapper: the blend-mode glyph with the editor's "active" highlight (the
 * teardrop fills in when the blend mode is non-default). The package
 * @grida/react-icons `BlendModeIcon` is the plain outline shape; the highlight
 * and the `active` state live here. See the package README ("shape-only").
 */
export function BlendModeIcon({
  ...
  return (
    <svg
      ...
      className={["group/icon", className].filter(Boolean).join(" ")}
      {...props}
    >
      <path
        fillRule="evenodd"
