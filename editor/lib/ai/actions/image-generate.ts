...
/**
 * AI image *generation* action — text → new pixels via the hosted media policy, with the generated bytes uploaded to the
...
export async function generateAiImage(
  ...
  const card = ai.image.findImageModelCard(input.model);
  ...
  if (
    ...
    !card.quality?.options.includes(input.quality)
  ) {
    return {
      success: false,
      code: "bad_request",
  ...
  return withAiAuth(
    ...
    async (organizationId) => {
      const generation = await methods.generateImage(organizationId, {
        model_id: card.id,
        prompt: input.prompt,
        width: input.width,
        height: input.height,
      ...
      const first = generation.images[0]!;
      const file = {
        uint8Array: Buffer.from(first.base64, "base64"),
        mediaType: first.media_type,
      };
      const { width, height } = imageSize(file.uint8Array);
