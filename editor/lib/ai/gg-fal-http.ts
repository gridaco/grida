// GRIDA-GG: gateway — fixed server egress for metered fal operations.
...
/** This host admits one selected fal endpoint and its billing receipt, never caller-selected authority. */
export namespace GgFalHttp {
  export function create(
    key: string,
    adminKey: string,
    endpoint: string
  ): ProviderHttp {
    if (!/^[a-zA-Z0-9_-]+(?:\/[a-zA-Z0-9_.-]+)+$/.test(endpoint)) fail();
    return new ProviderHttp({
      request: (input, init) =>
        execute("api", key, adminKey, endpoint, input, init),
      download: (input, init) =>
        execute("asset", key, adminKey, endpoint, input, init),
    });
  }
    ...
    try {
      ...
      if (lane === "api") {
        ...
        const poll =
          ...
          url.hostname === "queue.fal.run" &&
          !url.search &&
          [endpoint, root].some((scope) => {
            const prefix = `/${scope}/requests/`;
            return (
              url.pathname.startsWith(prefix) &&
              /^[A-Za-z0-9_-]{1,128}(?:\/(?:status|response))?$/.test(
        ...
        const receipt =
          method === "GET" &&
          url.hostname === "api.fal.ai" &&
          url.pathname === "/v1/models/billing-events" &&
          params.get("endpoint_id") === endpoint &&
          /^[A-Za-z0-9_-]{1,128}$/.test(params.get("request_id") ?? "") &&
          params.get("limit") === "2" &&
          [...params.keys()].length === 3;
        if (!(post || poll || receipt)) fail();
        if (headers.get("authorization") !== `Key ${receipt ? adminKey : key}`)
          fail();
        if (
          [...headers.keys()].some(
            (name) =>
        ...
        if (
          ...
          fail();
        if (post) {
          headers.set("x-fal-no-retry", "1");
          headers.set("x-app-fal-disable-fallback", "1");
        }
      } else {
        if (
          !(
