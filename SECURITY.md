...
Shared image operations and provider HTTP belong to
...
- [Standalone package proof](scripts/ai-local/proof.mjs), [runtime guard](scripts/ai-local/network.cjs), [public consumer](scripts/ai-local/consumer.mjs), and [guide](scripts/ai-local/README.md) — packed public exports with only declared production dependencies, synthetic transports, and guarded ambient network/credential/state access. No Grida host is installed or started.
...
6. **Custody doctrine (client half)** — the daemon holds the token in
   ...
   custody and image execution live in `@grida/ai`; native command composition
...
- [Standalone package proof](scripts/ai-local/proof.mjs), [runtime guard](scripts/ai-local/network.cjs), [public consumer](scripts/ai-local/consumer.mjs), and [guide](scripts/ai-local/README.md) — synthetic memory-only GG grants and fixed fake responses; proves package/credential routing without minting a real token or contacting a provider.
...
- [packages/grida-ai-agent/src/providers/gg.ts](packages/grida-ai-agent/src/providers/gg.ts) + [gg-media.ts](packages/grida-ai-agent/src/providers/gg-media.ts) — hosted text/video/music adapters consuming shared GG helpers: per-request token reads, editor-origin-only egress, code-led typed status errors (401→`gg_token_expired`, 402→`insufficient_credits`). Those constructed status messages omit upstream bodies; transport/parsing failures still require the calling operation's safe error boundary.
