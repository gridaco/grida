...
`/images`, `/video`, `/three-d`, `/audio`, the run loop and tools) is a **tenant** —
...
- [packages/grida-ai-agent/src/http/routes/generated-media-persistence.ts](packages/grida-ai-agent/src/http/routes/generated-media-persistence.ts), [images.ts](packages/grida-ai-agent/src/http/routes/images.ts), [video.ts](packages/grida-ai-agent/src/http/routes/video.ts), [three-d.ts](packages/grida-ai-agent/src/http/routes/three-d.ts), and [audio.ts](packages/grida-ai-agent/src/http/routes/audio.ts) — generated image/video/3D/audio bytes are best-effort persisted through the optional host media service. Only a neutral filename, MIME type, and bytes enter the store: prompts, provider/model identity, credentials, and native paths do not. A persistence failure is reduced to a generic warning and cannot discard or replace the already-generated response.
