import styled from "@emotion/styled";
import React from "react";

export default function SectionCtaLastSeeTheMagic1440SizeXl() {
  return (
    <RootWrapperSectionCtaLastSeeTheMagic1440SizeXl>
      <Frame270>
        <SeeTheMagic>See the magic</SeeTheMagic>
        <SeeWhatYouTruleyDoWithYourPowerSupercharged>
          See what you truley do with your power supercharged.
        </SeeWhatYouTruleyDoWithYourPowerSupercharged>
      </Frame270>
      <Frame554>
        <Frame549>
          <HttpsWwwFigmaComFileXxxxXxxxNodeId12343A5678>
            https://www.figma.com/file/xxxx/xxxx?node-id=1234%3A5678
          </HttpsWwwFigmaComFileXxxxXxxxNodeId12343A5678>
        </Frame549>
        <Frame550>
          <Frame551>
            <Image97
              src="https://s3-us-west-2.amazonaws.com/figma-alpha-api/img/4341/1554/128f94840d219df83cb481ca2ddd4a50"
              alt="image of Image97"
            ></Image97>
            <Abracadabra>Abracadabra</Abracadabra>
          </Frame551>
        </Frame550>
      </Frame554>
      <ScribbleGuide>
        <PasteYourFigmaDesignUrl>
          Paste your figma design url.
        </PasteYourFigmaDesignUrl>
        <PointerArtwork
          src="https://s3-us-west-2.amazonaws.com/figma-alpha-api/img/7b17/c650/db733369042b3472677812747401cb55"
          alt="image of PointerArtwork"
        ></PointerArtwork>
      </ScribbleGuide>
    </RootWrapperSectionCtaLastSeeTheMagic1440SizeXl>
  );
}

const RootWrapperSectionCtaLastSeeTheMagic1440SizeXl = styled.div`
  min-height: 100vh;
  background-color: rgba(255, 255, 255, 1);
  position: relative;
`;

const Frame270 = styled.div`
  display: flex;
  justify-content: flex-start;
  flex-direction: column;
  align-items: start;
  gap: 36px;
  box-sizing: border-box;
  position: absolute;
  left: calc((calc((50% + -1px)) - 328px));
  top: 105px;
  width: 655px;
  height: 137px;
`;

const SeeTheMagic = styled.span`
  color: rgba(0, 0, 0, 1);
  text-overflow: ellipsis;
  font-size: 64px;
  font-family: "Helvetica Neue", sans-serif;
  font-weight: 700;
  line-height: 98%;
  text-align: center;
  width: 655px;
`;

const SeeWhatYouTruleyDoWithYourPowerSupercharged = styled.span`
  color: rgba(68, 69, 69, 1);
  text-overflow: ellipsis;
  font-size: 24px;
  font-family: "Helvetica Neue", sans-serif;
  font-weight: 500;
  line-height: 160%;
  text-align: center;
  width: 655px;
`;

const Frame554 = styled.div`
  display: flex;
  justify-content: flex-start;
  flex-direction: row;
  align-items: start;
  gap: 24px;
  box-sizing: border-box;
  position: absolute;
  left: calc((calc((50% + 0px)) - 472px));
  top: 431px;
  width: 944px;
  height: 83px;
`;

const Frame549 = styled.div`
  width: 669px;
  height: 83px;
  overflow: hidden;
  background-color: rgba(255, 255, 255, 1);
  border-left: solid 1px rgba(210, 210, 210, 1);
  border-top: solid 1px rgba(210, 210, 210, 1);
  border-bottom: solid 1px rgba(210, 210, 210, 1);
  border-right: solid 1px rgba(210, 210, 210, 1);
  border-radius: 4px;
  position: relative;
  box-shadow: 0px 4px 48px rgba(0, 0, 0, 0.12);
`;

const HttpsWwwFigmaComFileXxxxXxxxNodeId12343A5678 = styled.span`
  color: rgba(210, 210, 210, 1);
  text-overflow: ellipsis;
  font-size: 21px;
  font-family: "Helvetica Neue", sans-serif;
  font-weight: 400;
  line-height: 160%;
  text-align: left;
  position: absolute;
  left: 28px;
  top: 25px;
`;

const Frame550 = styled.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  align-items: center;
  flex: none;
  gap: 10px;
  box-shadow: 0px 4px 48px rgba(0, 0, 0, 0.12);
  border-left: solid 1px rgba(210, 210, 210, 1);
  border-top: solid 1px rgba(210, 210, 210, 1);
  border-bottom: solid 1px rgba(210, 210, 210, 1);
  border-right: solid 1px rgba(210, 210, 210, 1);
  border-radius: 4px;
  width: 251px;
  height: 83px;
  background-color: rgba(0, 0, 0, 1);
  box-sizing: border-box;
  padding: 12px 12px;
`;

const Frame551 = styled.div`
  display: flex;
  justify-content: center;
  flex-direction: row;
  align-items: center;
  flex: none;
  gap: 8px;
  width: 191px;
  height: 46px;
  box-sizing: border-box;
`;

const Image97 = styled.img`
  width: 46px;
  height: 46px;
  object-fit: cover;
`;

const Abracadabra = styled.span`
  color: rgba(255, 255, 255, 1);
  text-overflow: ellipsis;
  font-size: 24px;
  font-family: "Helvetica Neue", sans-serif;
  font-weight: 500;
  letter-spacing: -1px;
  line-height: 98%;
  text-align: left;
`;

const ScribbleGuide = styled.div`
  width: 278px;
  height: 108px;
  position: absolute;
  left: calc((calc((50% + -408px)) - 139px));
  top: 306px;
`;

const PasteYourFigmaDesignUrl = styled.span`
  color: rgba(164, 164, 164, 1);
  text-overflow: ellipsis;
  font-size: 24px;
  font-family: "Nanum Pen Script", sans-serif;
  font-weight: 400;
  line-height: 98%;
  text-align: left;
  position: absolute;
  left: calc((calc((50% + -19px)) - 121px));
  top: 0px;
  transform: rotate(7deg);
`;

const PointerArtwork = styled.img`
  width: 75px;
  height: 84px;
  object-fit: cover;
  position: absolute;
  left: calc((calc((50% + 101px)) - 38px));
  top: 24px;
  transform: rotate(23deg);
`;
